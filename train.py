...
class Trainer:
    def __init__(self, config):
        ...
        self.enemies = [copy.deepcopy(self.agent) for _ in range(self.config.NUM_ENEMIES)]
    ...
    def play_game(self, enemy_index, first_player):
        ...
        while not winner:
            ...
            if self.env.player == 'agent':
                ...
                cur_player = 'enemy'
                action = self.enemies[enemy_index].get_action(-self.env.board, enemy_epsilon)
            ...
            if cur_player == 'agent':
                if winner == 'agent':
                    ...
                    self.visualize_transition(state, action, self.config.REWARD_WIN, next_state, done) if viz else None
                    self.agent.buffer.append((state, action, self.config.REWARD_WIN, next_state, done))
                elif winner == 'enemy':
                    # Agent lost by making an invalid move, apply lose reward
                    self.visualize_transition(state, action, self.config.REWARD_LOSE, next_state, done) if viz else None
                    self.agent.buffer.append((state, action, self.config.REWARD_LOSE, next_state, done))
                elif winner == 'draw':
                    # Agent drew, apply draw reward
                    self.visualize_transition(state, action, self.config.REWARD_DRAW, next_state, done) if viz else None
                    self.agent.buffer.append((state, action, self.config.REWARD_DRAW, next_state, done))
                elif winner == False:
                ...
                if delayed_buffer_update:
                    if winner == 'enemy':
                        ...
                        self.visualize_transition(*delayed_buffer_update, self.config.REWARD_LOSE, next_state, done) if viz else None
                        self.agent.buffer.append((*delayed_buffer_update, self.config.REWARD_LOSE, next_state, done))
                    elif winner == 'agent':
                        ...
                        self.visualize_transition(*delayed_buffer_update, self.config.REWARD_DRAW, next_state, done) if viz else None
                        self.agent.buffer.append((*delayed_buffer_update, self.config.REWARD_DRAW, next_state, done))
                    elif winner == False:
                        # Game is not over after enemy's move, apply step reward for agent
                        self.visualize_transition(*delayed_buffer_update, self.config.REWARD_STEP, next_state, done) if viz else None
                        self.agent.buffer.append((*delayed_buffer_update, self.config.REWARD_STEP, next_state, done))
    ...
    def train(self):
        for episode in range(self.config.EPISODES):
            ...
            first_player = 'agent' if (episode // self.config.NUM_ENEMIES) % 2 == 0 else 'enemy'
            ...
            if episode > 0 and episode % self.config.TOURNAMENT_FREQUENCY == 0 and len(self.all_past_enemies) >= self.config.TOURNAMENT_MIN_TOTAL_ENEMIES and self.config.TOURNAMENT_ENABLED:
                ...
                sorted_indices = sorted(range(len(tournament_results)), key=lambda x: tournament_results[x]['losses'], reverse=True)
                # Print the best 4 enemies
                print(f"\nBest {self.config.NUM_ENEMIES} enemies:")
                for i in range(self.config.NUM_ENEMIES):
                    print(f"Agent result VS Enemy {i}: {tournament_results[sorted_indices[i]]}")
                self.enemies = [tournament_enemies[i] for i in sorted_indices[:self.config.NUM_ENEMIES]]
            ...
            if episode > 0 and episode % self.config.EVALUATE_FREQUENCY == 0 and episode % self.config.TOURNAMENT_FREQUENCY != 0:
                ...
                for i, result in enumerate(self.agent_results_vs_enemies):
                    print(f"Agent stats vs enemy {i}: W: {result['wins']} - L: {result['losses']} - D: {result['draws']} - moved first: {self.agent_moved_first_vs_enemies[i]}")
    ...
    def hold_tournament(self, tournament_enemies):
        ...
        tournament_stats = [{'wins': 0, 'losses': 0, 'draws': 0} for _ in range(len(tournament_enemies))]
            ...
            for game_num in range(self.config.TOURNAMENT_GAMES):
                ...
                while not winner:
                    if game.player == 'agent':
                        action = self.agent.get_action(game.board, torunament_epsilon)
                        if action not in game.get_valid_actions():
                            winner = 'enemy'
                    else:
                        action = tournament_enemies[enemy_index].get_action(-game.board, torunament_epsilon)
                        if action not in game.get_valid_actions():
                            winner = 'agent'
    ...
    def visualize_transition(self, state, action, reward, next_state, done):
        ...
        visual_state = np.where(state == 1, 'A', np.where(state == -1, 'E', state))
        visual_next_state = np.where(next_state == 1, 'A', np.where(next_state == -1, 'E', next_state))
