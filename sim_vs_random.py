...
def run_simulations():
    ...
    for model_file in sorted(model_files, key=lambda x: int(x.split('model_')[1].split('.pt')[0])):
        ...
        for agent_moves_first, label in [(True, 'first'), (False, 'second')]:
            win_rate, invalid_move_rate, avg_moves = run_simulation(agent, Config, agent_moves_first)
            data_point = {"episode_num": episodes, "win_rate": win_rate, "invalid_move_rate": invalid_move_rate, "avg_moves": avg_moves}
            for key in data_point.keys():
                if key != 'episode_num':
                    data[key + '_when_moving_' + label].append((episodes, data_point[key]))
    plot_rates(data)
    print("Done")
...
def plot_rates(data):
    ...
    for i, label in enumerate(labels):
        for key, subplot in zip(['win_rate', 'invalid_move_rate', 'avg_moves'], axs[:, i]):
            ...
            subplot.plot(episodes, rates, marker='o', label=f'Agent {label.replace("_", " ").title()}')
