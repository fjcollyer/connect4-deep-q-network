...
def get_latest_saved_model_path():
    files = [file for file in os.listdir(Config.SAVE_FOLDER) if file.endswith(".pt")]
...
def draw_board(board, q_values=None):
    ...
    if q_values is not None:
        for idx, value in enumerate(q_values):
            plt.text(idx, -1.2, f"{value:.3f}", horizontalalignment='center', verticalalignment='center', color='black', fontweight='bold')
...
def play_game(mode):
    ...
    if mode in ["me1", "me2"]:
        ...
        print(f"You are playing against the AI! You are 'enemy' (represented by -1), AI is 'agent' (represented by 1). You are {'moving first' if mode == 'me1' else 'moving second'}.")
        winner = False
        while not winner:
            draw_board(env.board, agent.get_q_values(-env.board))

            if env.player == 'enemy':
                action = int(input(f"Enter the column number (1-{config.COLUMNS}) where you want to drop your piece: ")) - 1
            else:
                ...
                # Confirmation step
                confirm = input("Press 'y' to let the agent make its move: ").lower()
                while confirm != 'y':
                    confirm = input("Press 'y' to let the agent make its move: ").lower()
        ...
        while not winner:
            ...
            if env.player == 'agent':
                ...
                action = agent.get_action(-env.board, 0)  # Flip the board to represent the enemy's perspective
        ...
        print(f"Random mode activated. {'Agent' if starting_player == 'agent' else 'Enemy'} is moving first.")
